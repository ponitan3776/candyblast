import { RANKING_MODES, RANKING_MODE_LABELS, RANKING_TYPES, RANKING_TYPE_LABELS } from './config.js';
import { fetchRanking, syncFromServer } from './api.js';
import { authToken, currentUserId, modalContent } from './game-core.js';
...
export function renderRankingModal() {
  ...
  // データ取得
  fetchRankingData(currentRankingMode, currentRankingType).then(htmlContent => {
    document.getElementById('rankingContent').innerHTML = htmlContent;
  }).catch(() => {
    document.getElementById('rankingContent').innerHTML = `<div class="sub" style="color:var(--coral);">❌ ランキングの読み込みに失敗しました</div>`;
  });
}

async function fetchRankingData(mode, type) {
  try {
    const data = await fetchRanking(mode, type);
    let content = `<div style="text-align:left; max-height:380px; overflow-y:auto;">`;
    if (data.top && data.top.length > 0) {
      data.top.forEach((user, index) => {
        const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index+1}.`;
        const isMe = user.id === currentUserId;
        const valueLabel = type === 'coins' ? `${user.value}コイン` :
                           type === 'playtime' ? `${user.value}秒` :
                           `${user.value}点`;
        content += `
      ...
      const msg = type === 'score' ? 'このモードのランキングデータがまだありません。' :
                  type === 'coins' ? 'まだコインデータがありません。' :
                  'プレイ時間データがまだありません。';
    ...
    if (authToken && currentUserId) {
      let myValue = 0;
      try {
        const syncData = await syncFromServer(authToken);
        if (syncData) {
          if (type === 'coins') {
      ...
      if (myValue > 0) {
        ...
        const valueLabel = type === 'coins' ? `${myValue}コイン` :
                           type === 'playtime' ? `${myValue}秒` :
                           `${myValue}点`;
        content += `
        ...
        const msg = type === 'score' ? 'まだこのモードのスコアがありません。' :
                    type === 'coins' ? 'まだコインがありません。' :
                    'プレイ時間が記録されていません。';
        content += `
